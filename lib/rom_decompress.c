/*
 * rom_decompress.c: generate a decompressed layout from a ROM
...
decompress_rom(struct zelda64_rom const* rom,
               struct zelda64_allocator const allocator,
               struct zelda64_error* error) {
    // Work our way over the entries first.
...
zelda64_decompress(struct zelda64_rom const* rom, struct zelda64_error* error) {
    ...
    return zelda64_decompress_with_allocator(rom, rom->allocator, error);
}
...
zelda64_decompress_with_allocator(struct zelda64_rom const* rom,
    ...
    struct zelda64_layout_entry* entries = decompress_rom(rom, allocator, error);
    if (entries == NULL) {
        return NULL;
    }

    // Allocate a data for our layout.
