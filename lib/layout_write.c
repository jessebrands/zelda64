...
write_rom(struct zelda64_io* out_rom,
    ...
    if (options->pad != ZELDA64_PAD_NONE) {
        rom_size += pad_file(out_rom, (uint32_t) rom_size, options->pad, error);
        if (ZELDA64_FAILED(error)) {
            zelda64_free(layout->allocator, dmadata);
            return 0;
        }
...
zelda64_write(char const* filename,
    ...
    if (ZELDA64_FAILED(error)) {
        zelda64_io_close(&out_file, NULL);
        return 0;
    }

    zelda64_io_close(&out_file, error);
    return bytes_out;
}
...
zelda64_write_buffer(uint8_t* data, size_t const size,
    ...
    if (ZELDA64_FAILED(error)) {
        zelda64_io_close(&out_buffer, NULL);
        return 0;
    }

    zelda64_io_close(&out_buffer, error);
    return bytes_out;
}
